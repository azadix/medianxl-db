export class DropdownList {
    ...
    renderItems(items = this.items) {
        ...
        Object.entries(groupedItems).forEach(([type, groupItems]) => {
            ...
            const label =
                this.options.headerLabels[type] || type || this.options.defaultHeaderText;
    ...
    showList() {
        ...
        while (currentElement && currentElement !== document.body) {
            if (
                currentElement.id === 'oskill-dropdown' ||
    ...
    positionOSkillsDropdown() {
        ...
        const leftPos = inputRect.left + inputRect.width;
        const topPos = inputRect.bottom - Math.floor(dropdownHeight / 2 + inputRect.height / 2);
