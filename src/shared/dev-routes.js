/**
 * @file Dev-only Vue routes (editor). Imported by router and navbar.
 * @module shared/dev-routes
 */

/** @typedef {{ path: string, name: string, component: () => Promise<unknown>, keepAliveName: string, navLabel: string, keepAlive?: boolean, editorMode?: string, editorFile?: string }} DevRouteDef */
...
export const DEV_ROUTE_DEFS = [
  {
    ...
    keepAliveName: 'SkillsEditorView',
    // Shared editor-store singleton must not outlive a keep-alive cache entry.
    keepAlive: false,
    navLabel: 'Editor',
    editorMode: 'skills',
  ...
  },
  {
    ...
    component: () => import('@/views/EditorView.vue'),
    keepAliveName: 'SubskillsEditorView',
    keepAlive: false,
    navLabel: 'Subskills',
...
export const DEV_KEEP_ALIVE_NAMES = [
  ...new Set(
    DEV_ROUTE_DEFS.filter((r) => r.keepAlive !== false).map((r) => r.keepAliveName)
  ),
];
