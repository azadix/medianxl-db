...
import { refreshCurrentTooltip } from './tree-tooltip.js';
...
export async function initializeTreePage() {
    ...
    classSelect.addEventListener('change', function onPlannerClassSelectChange() {
        ...
        if (oskillDropdown) oskillDropdown.value = '';
        if (window.oskillDropdownInstance) window.oskillDropdownInstance.renderItems();
