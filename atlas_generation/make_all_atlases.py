...
def main():
    parser = argparse.ArgumentParser(
        description="Build class-*.png atlas sprites under public/tree_data/<major>_<minor>/ from per-icon frames."
    )
    parser.add_argument(
        "--version",
        required=True,
        help='Game patch, e.g. "2.12" (output: public/tree_data/2_12/class-<class>.png)',
    )
    ...
    repo_root = os.path.dirname(icons_dir)
    ...
    for class_id in CLASS_IDS:
        ...
        output_file = os.path.join(out_dir, f"class-{class_id}.png")
